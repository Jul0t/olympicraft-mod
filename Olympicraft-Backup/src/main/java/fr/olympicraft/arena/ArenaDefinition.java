...
public final class ArenaDefinition {
    ...
    public Map<String, ArenaPosition> spawns =
            new LinkedHashMap<>();
    ...
    public GameType resolvedGameType() {
        return GameType.fromInput(gameType).orElse(null);
    }
    ...
    public ArenaValidationResult validate() {
        ...
        if (resolvedGameType() == null) {
            result.error(
                    "Le type de mini-jeu est inconnu : "
                            + gameType
            );
        }

        if (worldId == null || worldId.isBlank()) {
            result.error("Le monde de l'arène est inconnu.");
        }
